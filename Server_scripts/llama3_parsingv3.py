...
def send_api_request(references, first_batch=False):
    ...
    if first_batch:
        ...
        prompt_text = (
            "Continue parsing these next references into the existing metadata table. Use the following headers: "
            "| Authors | Year | Title | Journal/Book Name | Volume/Issue | Page Range | Location |\n"
            "| --- | --- | --- | --- | --- | --- | --- |\n\n"
            "References:\n"
            f"{references}\n\n"
            "Use the same matrix format with metadata fields split by vertical lines. Do not number the references in the table. "
            "Do not add any columns. Do not abbreviate any metadata. If no metadata is found, just leave the field empty. "
            "Make sure the table field fits the header (volumes/issues or Page range can't be only text)."
        )
